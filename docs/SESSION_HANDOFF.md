...
## 2026-08-05 Current Handoff: Public Repo and Tasks UI Review

Current repo status:

- Public GitHub push has been completed.
...
- Public repo excludes local data, local artifacts, private configs, generated dependencies/build outputs, and local-only helper projects.

Current app status:

- Local Backend: `http://127.0.0.1:8000`
- Local Frontend: `http://127.0.0.1:3000/tasks`
- Web App live workflow has already been proven usable in the browser.
- Current focus is frontend layout and visual review, not revalidating the full workflow.

Current UI status:

- Existing shared-component cleanup and first full-route consistency pass are complete.
- Available/installed design skills for the next UI work:
  - `frontend-design`
  - `web-design-guidelines`
  - `ui-ux-pro-max`, installed at `/home/votally/.codex/skills/ui-ux-pro-max`
- `/tasks` page has just received a layout split:
  - Create-task fields are always visible in a dedicated top section.
  - The old collapsible create-task trigger has been removed.
  - Search has moved into the existing-task section.
  - Search only filters the existing task/test task list.
  - Existing-task metrics are grouped with the existing-task section.
- Changed frontend file: `webapp/frontend/src/features/tasks/task-list-page.tsx`.
...
Important next-conversation instruction:

- Do not modify code immediately.
- First read the documented project state and the current `/tasks` implementation.
- Then summarize the current status for the user.
- Wait for the user to give more `/tasks` page layout or interaction feedback before changing files.

Standing constraints:

- Do not start a new collection run unless the user explicitly confirms it.
- Do not run model-backed stages unless the user explicitly agrees.
- Keep Web App work under `webapp/`.
- Preserve the existing Python analysis pipeline under `jobuwant/` and wrap it through service/API boundaries when needed.
## 2026-08-05 Create Task City Selector Handoff

Current `/tasks` create form status:

- Visible inputs are now task name, city selector, target role field labeled `查找岗位`, job type, and target count.
- City code is hidden from the user and inferred by the backend from the supported-city catalog.
- Source type is hidden and still auto-generated by the backend as `webapp_task_{id}` when empty.
- Batch size is hidden in the form and submitted as 10.
- Target count is limited to 200; the frontend shows `最多不超过 200 条，请减少目标数量` and blocks submission when exceeded.
- Backend schema also limits `expected_job_count` to 200.
- New endpoint: `GET /api/cities`.
- New city catalog: `webapp/backend/app/core/city_catalog.py`.
- Hangzhou, Guangzhou, and Shenzhen remain the locally verified city-code entries.
...
- Do not start a new collection run unless the user explicitly confirms it.
- Do not run model-backed stages unless the user explicitly agrees.
## 2026-08-06 Tasks Page Layout Handoff

Current `/tasks` page layout:

- Top header title: `JobUWant——一键查询分析岗位工具`.
- Right header action: compact status pill.
- Main create-task area is centered and emphasized, with explanatory copy above the form.
- Visible form fields remain task name, city, role, job type, and target count.
- Lower task history is collapsible and shows at most the latest 3 tasks.
- Button submit and task navigation behavior were not changed in this pass.
- Frontend preview was rebuilt and restarted on `http://127.0.0.1:3000/tasks`.

## 2026-08-06 Web App Cancellation Boundary Handoff

Current update:

- Live task cancellation boundary is implemented through `POST /api/tasks/{task_id}/actions/cancel`.
- Canceling a live running task marks the active stage as `canceled`, pending downstream stages as `skipped`, task status as `canceled`, and appends `task_canceled`.
- The collection runner now tracks cancellation requests and active subprocesses. When a collection is canceled before completion, it removes the generated collection output and does not import or record a `search_run` artifact.
- Frontend API client now exposes `cancelTask`, but no visible cancel button has been wired yet.
- No real collection run was started and no model-backed stage was called.

Verification:

- Backend tests: 41 passed, 25 warnings.
- Frontend typecheck passed through direct TypeScript invocation.
- Frontend lint passed through direct ESLint invocation.
- Frontend build was attempted but stopped after more than 9 minutes without completion in the Windows Node plus WSL path setup.

Recommended next step:

- Implement the frontend running-state UI: `/tasks` primary action should create and start collection, show “正在搜集岗位信息”, call `cancelTask` from the interruption button, and keep model-backed analysis behind a later explicit “确定并开始分析” action.
## 2026-08-06 Start-Find Frontend Flow Handoff

Current update:

- `/tasks` primary button now shows `开始查找`.
- Clicking it creates a live task, starts collection immediately, and opens `/tasks/{taskId}`.
- Task detail now shows collection-running copy and a `中断查找` button that calls `POST /api/tasks/{task_id}/actions/cancel`.
- Once collection completes, task detail automatically runs local scoring and then routes to `/tasks/{taskId}/sample`.
- No model-backed stage is triggered by this flow.
...
- Update sample confirmation so the main button becomes `确定并开始分析`, then save the sample and move into the analysis-stage flow with explicit model/cost confirmation before model-backed execution.

## 2026-08-13 UNC UTF-8 Editing Note

A Windows PowerShell edit against `webapp/frontend/src/features/tasks/task-list-page.tsx` damaged Chinese UTF-8 strings because `Get-Content -Raw` used the Windows code page before the file was written back as UTF-8. Symptoms included mojibake Chinese text, Unicode replacement characters (`U+FFFD`), broken string literals, and a missing JSX `</p>` parse error.

Use this safer workflow for future frontend edits in the WSL UNC workspace:

1. Prefer `apply_patch` when it works.
2. If `apply_patch` cannot access the UNC path, prefer WSL-side writes through a small temporary script and `Path.write_text(..., encoding='utf-8')`.
3. Do not use Windows PowerShell `Get-Content`/`Set-Content` on TSX files unless explicit UTF-8 read and write APIs are used.
4. If Windows PowerShell must write a UTF-8 file, use .NET APIs with explicit UTF-8:
   - `[System.IO.File]::ReadAllText($path, [System.Text.Encoding]::UTF8)`
   - `[System.IO.File]::WriteAllText($path, $text, [System.Text.UTF8Encoding]::new($false))`
5. After the write, inspect for replacement characters, run frontend TypeScript, run ESLint, and then `npm run build` before restarting preview.
